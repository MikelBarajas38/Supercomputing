...
{   
    ...
    {
        displs[i] = i * stride;
        counts[i] = i == numproc-1 ? nDataLast : nData;
    }
    ...
    Aproc = createArray(nData);
    Bproc = createArray(nData);
    Cproc = createArray(nData);

    MPI_Scatterv(A, counts, displs, MPI_INT, Aproc, nData, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Scatterv(B, counts, displs, MPI_INT, Bproc, nData, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_Gatherv(Cproc, nData, MPI_INT, C, counts, displs, MPI_INT, 0, MPI_COMM_WORLD);
